...
class Hyperparameters:
    ...
    mixture_head_enabled = bool(int(os.environ.get("MIXTURE_HEAD_ENABLED", "1")))
...
def _hash_context_np(ctx: np.ndarray) -> int:
    h = 0
    for v in ctx:
        h = (h * 1_000_003 + int(v) + 1) & 0xFFFFFFFFFFFFFFFF
    return int(h)


def build_mixture_artifact_log_probs(
    train_glob: str,
    vocab_size: int,
    num_buckets: int,
    max_tokens: int,
    *,
    ...
    """Hashed order 2..6 n-gram marginal log P(tok | bucket) from training shards (Dirichlet smoothing)."""
    files = sorted(glob.glob(train_glob))
    if not files:
        raise FileNotFoundError(f"No train files for artifact build: {train_glob}")
    counts = np.zeros((num_buckets, vocab_size), dtype=np.float64)
    seen = 0
    orders = (2, 3, 4, 5, 6)
    min_o = max(orders)
    for fp in files:
        if seen >= max_tokens:
            break
        toks = load_data_shard(Path(fp)).numpy().astype(np.int64)
        n = int(toks.shape[0])
        for t in range(min_o, n):
            if seen >= max_tokens:
                break
            w = int(toks[t])
            if w < 0 or w >= vocab_size:
                seen += 1
                continue
            for o in orders:
                ctx = toks[t - o : t]
                h = _hash_context_np(ctx) % num_buckets
                counts[h, w] += 1.0
            seen += 1
    row_sum = counts.sum(1, keepdims=True) + alpha * float(vocab_size)
    with np.errstate(divide="ignore"):
        logp = np.log((counts + alpha) / row_sum)
    logp = np.nan_to_num(logp, nan=-math.log(vocab_size), neginf=-math.log(vocab_size))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,mixture_gate,artifact_log_probs",
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        if mixture_head_enabled:
            ...
            self.mixture_gate = nn.Sequential(
                nn.Linear(gate_in, mixture_gate_hidden),
                nn.SiLU(),
                nn.Linear(mixture_gate_hidden, 3),
            )
    ...
    def _init_weights(self) -> None:
        ...
        if self.mixture_gate is not None:
            last = self.mixture_gate[2]
    ...
    def _lm_logits_from_hidden(self, x: Tensor) -> Tensor:
        if self.tie_embeddings:
    ...
    def _artifact_bucket_ids(self, input_ids: Tensor) -> Tensor:
        B, T = input_ids.shape
        out = torch.zeros(B, T, dtype=torch.int64, device=input_ids.device)
        x = input_ids.long()
        for t in range(T):
            start = max(0, t - 6)
            ctx = x[:, start:t]
            h = torch.zeros(B, dtype=torch.int64, device=x.device)
...
def main() -> None:
    ...
    if args.mixture_head_enabled:
        vn = args.vocab_size
        if args.mixture_artifact_max_tokens > 0:
            art_t = torch.zeros(
                args.mixture_artifact_buckets, vn, dtype=torch.float32, device=device
            )
            if master_process:
                art_t.copy_(
                    build_mixture_artifact_log_probs(
                        args.train_files,
                        vn,
                        args.mixture_artifact_buckets,
                        args.mixture_artifact_max_tokens,
                    ).to(device)
                )
            if dist.is_available() and dist.is_initialized():
                dist.broadcast(art_t, src=0)
            base_model.artifact_log_probs.copy_(art_t)
        base_model.tok_is_ws[:vn].copy_(has_leading_space_lut[:vn].float())
        base_model.tok_is_boundary[:vn].copy_(is_boundary_token_lut[:vn].float())
    ...
    if args.mixture_head_enabled:
        vn = args.vocab_size
        eval_model.tok_is_ws[:vn].copy_(has_leading_space_lut[:vn].float())
        eval_model.tok_is_boundary[:vn].copy_(is_boundary_token_lut[:vn].float())
